...
- MCU 状态为 `WAIT_MANUAL_CUP_CLEAN` 时，表示人工清洗接液烧杯并补加 10ml 完成。
- MCU 状态为 `POWER_ON_RESET`，且因为寻找 PG3 超时停在上电复位人工修正等待中时，表示用户已经处理好 Z 轴机械位置，允许 MCU 重新执行上电复位。
...
如果 `warn` 页面用于上电复位 Y 轴异常，不需要发送 `#OK;`。MCU 会自动轮询 PG1，PG1 消抖有效并持续 1s 后自动恢复复位。
...
- `ASP_MS` 和 `TRIM10_MS` 不保存，断电或复位后恢复 `My/app_config.h` 中默认值。新的分阶段吸取停留时间使用 `APP_ASP_DWELL_800_MS` 到 `APP_ASP_DWELL_50_MS`，暂不由 HMI 修改；这些值表示 100% 满速泵下的标定时间，自动运行时会按当前 `#SPD` 速度自动换算。
...
如果该异常发生在上电复位过程中，MCU 不会进入普通 `ERROR` 结束态，而是保持 `POWER_ON_RESET` 状态并自动轮询 PG1。PG1 消抖有效并持续 1s 后，MCU 会自动重新执行上电复位，`warn` 页面不需要发送 `#OK;`。

如果上电复位向上寻找 PG3 超时，MCU 会报警 `Z_TIMEOUT`、跳转 `warn` 页面，并同样等待用户修正后发送 `#OK;` 继续复位。
...
| 4 | `Z_TIMEOUT` | Z 轴到位超时；上电复位找 PG3 超时时会跳转 `warn` 并等待确认 |
| 5 | `BAD_COMMAND` | 命令无效 |
| 6 | `BAD_CONFIG` | 固件体积或喷淋配置错误 |
| 7 | `SAVE_FAILED` | Flash 保存喷淋补偿时间失败 |
| 8 | `LEAK_DETECTED` | 漏水检测异常；正式模式下会停止全部动作并跳转 `leak_warn` 页面 |
...
- 普通自动/手动 Z 轴动作中 PG1 无效时，立即停止全部执行机构，进入 `ERROR`，报警码为 `Y_NOT_READY`，并跳转到 `warn` 页面。
- 上电复位中 PG1 无效时，立即停止全部执行机构，保持 `POWER_ON_RESET`，报警码为 `Y_NOT_READY`，并跳转到 `warn` 页面；MCU 会定时轮询 PG1，PG1 消抖有效并持续 1s 后自动重新复位。
- 上电复位中寻找 PG3 超时时，立即停止全部执行机构，保持 `POWER_ON_RESET`，报警码为 `Z_TIMEOUT`，并跳转到 `warn` 页面等待 `#OK;` 后重新复位。

漏水保护规则：

- 漏水检测使用 PG8 原始电平，不使用普通 PG 的低电平有效触发语义。
- PG8 高电平表示正常。
- PG8 低电平表示漏水异常。
- 当前默认正式模式，连续低电平异常达到防抖次数后，MCU 停止全部执行机构，报警码为 `LEAK_DETECTED`，并跳转到 `leak_warn` 页面。
...
- `APP_Y_READY_RESUME_CHECK_MS`：上电复位因 PG1 无效暂停后，检查 PG1 的周期，默认 `100ms`。
- `APP_Y_READY_RESUME_DEBOUNCE_COUNT`：PG1 连续有效计数，默认 `3` 次。
- `APP_Y_READY_RESUME_HOLD_MS`：PG1 消抖有效后继续保持的时间，默认 `1000ms`，到时自动重新执行上电复位。
...
- 吸取位置的停留时间如果设为 `0U`，且下一段仍然同方向下行，状态机会连续通过该位置，不主动刹车、不进入停留吸取状态。
- 然后从 200ml 开始进入真实 PG 定位吸取段。
- 真实段下降找 PG 时泵已经开始吸取；到达 PG 后继续停留吸取该段默认时间。
- 200/150/100ml 真实 PG 位置在停留时间为 `0U` 且下一段仍继续下行时，也会像虚拟位置一样连续通过。
- 最终目标位置不会连续通过；例如 50ml/PG7 作为最终目标时会停止，不会继续向下。
