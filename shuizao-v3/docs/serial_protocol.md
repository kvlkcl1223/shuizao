...
- MCU 状态为 `POWER_ON_RESET` 且已经停在上电复位人工修正等待中时，表示用户已经处理好机械位置，允许 MCU 重新执行上电复位。

屏幕应在人工清洗/补加提示页、上电复位 `warn` 页面确认按钮中发送：
...
如果该异常发生在上电复位过程中，MCU 不会进入普通 `ERROR` 结束态，而是保持 `POWER_ON_RESET` 状态并等待人工修正。用户修正后，`warn` 页面确认按钮发送 `#OK;`，MCU 会重新执行上电复位。
...
- 上电复位中 PG1 无效时，立即停止全部执行机构，保持 `POWER_ON_RESET`，报警码为 `Y_NOT_READY`，并跳转到 `warn` 页面等待 `#OK;` 后重新复位。
